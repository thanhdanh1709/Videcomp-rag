...
import { VidecompLogo } from "./Icons";
...
export function AdminLoginModal({
  ...
  isOpen: boolean;
  onClose: () => void;
  onLogin: (username: string, pass: string) => Promise<{ success: boolean; role?: "admin" | "user"; message?: string }>;
  onRegister: (data: {
    username: string;
    email: string;
    password: string;
  ...
  // Login form state (KHÔNG ĐIỀN SẴN DỮ LIỆU)
  ...
  // Validation kiểm tra mật khẩu real-time
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    if (!isPasswordValid) {
      setErrorMsg("Mật khẩu chưa đáp ứng đầy đủ các tiêu chuẩn bảo mật bên dưới.");
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        onShowToast?.("Đăng ký tài khoản thành công! Đã tự động đăng nhập vào phòng Chat.");
        onClose();
      } else {
        setErrorMsg(res.message || "Đăng ký không thành công. Vui lòng kiểm tra lại thông tin.");
      }
    } catch (err: any) {
      setErrorMsg(err.message || "Đã xảy ra lỗi trong quá trình tạo tài khoản.");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-[#080808] text-[#ececf1] font-sans flex flex-col justify-between selection:bg-[#10a37f]/30 selection:text-[#1cd6a8] animate-in fade-in duration-200">
      ...
      {/* Top Navigation Bar */}
      <header className="relative z-10 w-full px-6 py-4 sm:py-5 flex items-center justify-between border-b border-white/5 bg-[#080808]/70 backdrop-blur-md">
        <div
          onClick={onClose}
      ...
      <main className="relative z-10 flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 lg:py-10 flex items-center justify-center">
        <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
          ...
          <div className="lg:col-span-6 flex flex-col justify-center space-y-8 pr-0 lg:pr-4">
            ...
            {/* Cinematic Preview Card (Like GetLayers 3D / Shader card) */}
                ...
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h2 className="text-2xl font-bold text-white tracking-tight">
                      {tab === "login" ? "Chào mừng trở lại" : "Khởi tạo tài khoản"}
                    </h2>
                  ...
                  <button
                    type="button"
                    onClick={onClose}
                    className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 border border-white/10 transition-colors"
                    title="Đóng cửa sổ"
                  >
